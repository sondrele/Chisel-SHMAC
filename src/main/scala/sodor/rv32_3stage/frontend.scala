...
class FrontEnd(implicit conf: SodorConfiguration) extends Module
{
   ...
   {
      when ((fetch && io.imem.req.ready) || io.cpu.req.valid)
      {
   ...
   io.imem.resp.ready    := io.cpu.resp.ready
   ...
   when (io.cpu.resp.ready)
   {
      exe_reg_valid := io.imem.resp.valid
      exe_reg_pc    := io.imem.resp.bits.addr
