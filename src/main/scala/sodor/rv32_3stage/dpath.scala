...
class DatPath(implicit conf: SodorConfiguration) extends Module 
{
   ...
   val ls_wait = Reg(init=Bool(false))
   ...
   wb_hazard_stall := ((wb_reg_wbaddr === exe_rs1_addr) && (exe_rs1_addr != UInt(0)) && wb_reg_ctrl.rf_wen && !wb_reg_ctrl.bypassable) ||
      ...
      ((io.dmem.req.valid && !io.dmem.req.ready && !io.dmem.resp.valid)) || // memory not ready to accept request
      (io.dmem.req.valid || (ls_wait && !io.dmem.resp.valid)) // waiting for mem response

   // Register File
   val regfile = Mem(Bits(width = conf.xprlen), 32)

   val write_regfile = wb_reg_ctrl.rf_wen && wb_reg_wbaddr != UInt(0)
